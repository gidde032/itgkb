...
const constellations = JSON.parse(
  readFileSync(join(root, 'content', 'constellations.json'), 'utf8'),
);
...
for (const file of readdirSync(articlesDir)
  ...
  errors = errors.concat(validateFilenameId(file, fm.id, file));
  entries.push({ fm, sourceName: file });
}

errors = errors.concat(validateConstellations(constellations));
...
console.log(
  `Content validation passed: ${entries.length} articles, ${constellations.length} constellations.`,
);
